...
async function authlibRoutes(fastify: FastifyInstance, opts: Options) {
    ...
    fastify.post<{ Body: JoinRequestDto }>(
        "/authlib/sessionserver/session/minecraft/join",
        async (req, reply) => {
            ...
            if (!status)
                return reply.code(400).send("ForbiddenOperationException: Invalid credentials.");
            reply.code(200);
        },
    );

    fastify.get<{ Querystring: QueryHasJoinedDto }>(
        "/authlib/sessionserver/session/minecraft/hasJoined",
        async (req, reply) => {
    ...
    fastify.get<{ Params: ParamsProfile; Querystring: QueryProfile }>(
        "/authlib/sessionserver/session/minecraft/profile/:uuid",
        async (req, reply) => {
            const user = await opts.authProvider.profile(UUIDHelper.getWithDashes(req.params.uuid));
            if (!user)
                return reply.code(204).send("ForbiddenOperationException: Invalid credentials.");
            const textures: any = {};
            if (user.skinUrl?.length > 0) {
                textures.SKIN = {
                    url: user.skinUrl,
            ...
            if (req.query.unsigned)
                data.properties[0].signature = opts.authlibManager.getSignature(texturesValue);
            reply.send(data);
        },
    );
}
